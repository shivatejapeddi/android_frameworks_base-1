...
import android.miui.BiometricConnect.DBGroup;
import android.miui.BiometricConnect.DBTemplate;
...
public class Miui3DFaceManagerImpl implements IMiuiFaceManager,  BiometricClient.ServiceCallback {
    ...
    private static String LOG_TAG = "3DFaceManagerImpl_clientFaceManagerImpl_client";
    ...
    private EnrollParam mEnrollParam = new EnrollParam();
    ...
    private List<Miuiface> mMiuifaceList = null;
    ...
    private void preloadBoostFramework() {
        try {
            ...
            if (this.sPerfClass != null) {
                Constructor constuctor = this.sPerfClass.getConstructor(new Class[0]);
                if (constuctor != null) {
                    this.boostFramework = constuctor.newInstance(new Object[0]);
                }
                this.sAcquireFunc = this.sPerfClass.getMethod("perfLockAcquire", new Class[]{Integer.TYPE, int[].class});
                this.sReleaseFunc = this.sPerfClass.getMethod("perfLockRelease", new Class[0]);
    ...
    private void handlerDatabase(Bundle bundle) {
        ...
        ArrayList<DBGroup> listGroup = bundle.getParcelableArrayList("group");
        if (BiometricConnect.DEBUG_LOG) {
            String str = LOG_TAG;
            StringBuilder stringBuilder = new StringBuilder();
        ...
        ArrayList<DBTemplate> listTemplate = bundle.getParcelableArrayList(BiometricConnect.MSG_CB_BUNDLE_DB_TEMPLATE);
        if (BiometricConnect.DEBUG_LOG) {
            String str2 = LOG_TAG;
    ...
    private void initClientDB(ArrayList<DBGroup> listGroup, ArrayList<DBTemplate> list) {
        if (this.mDatabaseStatus != 2 || this.mDatabaseChanged) {
            Log.d(LOG_TAG, "initClientDB begin");
            this.mTemplateItemList = new ArrayList();
            Iterator it = list.iterator();
            while (it.hasNext()) {
                DBTemplate i = (DBTemplate) it.next();
                TemplateItem item = new TemplateItem();
                item.id = i.mId;
                item.name = i.mName;
                item.group_id = i.mGroupId;
                item.data = i.mData;
            ...
            while (it.hasNext()) {
                DBGroup group = (DBGroup) it.next();
                GroupItem groupItem = new GroupItem();
                groupItem.id = group.mId;
                groupItem.name = group.mName;
    ...
    private void commitDatabase() {
        if (this.mReleased) {
            ...
            ArrayList<DBGroup> listGroup = new ArrayList();
            for (GroupItem g : this.mGroupItemList) {
                listGroup.add(new DBGroup(g.id, g.name));
            }
            out_bundle.putParcelableArrayList("group", listGroup);
            ArrayList<DBTemplate> listTemplate = new ArrayList();
            for (TemplateItem i : this.mTemplateItemList) {
                listTemplate.add(new DBTemplate(i.id, i.name, i.data, i.group_id));
            }
    ...
    private void cancelAuthentication() {
        ...
        try {
            if (this.sReleaseFunc != null) {
                this.sReleaseFunc.invoke(this.boostFramework, new Object[0]);
            }
    ...
    public void authenticate(CancellationSignal cancel, int flags, AuthenticationCallback callback, Handler handler, int timeout) {
        ...
        if (this.mReleased) {
            ...
            handler2.sendMessageDelayed(handler2.obtainMessage(1), (long) timeout);
        } else {
    ...
    public void enroll(byte[] cryptoToken, CancellationSignal cancel, int flags, EnrollmentCallback enrollCallback, Surface surface, RectF detectArea, RectF enrollArea, int timeout) {
        ...
        handler.sendMessageDelayed(handler.obtainMessage(i), (long) timeout2);
    }
